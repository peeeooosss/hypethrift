...
export default async function ListingsIndexPage({
  ...
  searchParams: Promise<{ q?: string; cat?: string; sizes?: string; mode?: string }>;
}) {
  const params = await searchParams;
  const { q, cat, sizes: sizesParam } = params;
  const mode = params.mode === "retail" ? "retail" : "auction";
  const retailFilter = mode === "retail" ? { listingMode: { in: ["RETAIL" as const, "BOTH" as const] }, buyNowPrice: { not: null } } : {};
    ...
    <div className="min-h-screen bg-cream text-ink py-10 pb-24">
      <div className="max-w-7xl mx-auto px-4">
        <h1 className="text-4xl font-black uppercase mb-4">
          {mode === "auction" ? "Live Auctions" : "Buy Now"}
        </h1>

        <div className="flex flex-wrap items-center gap-2 mb-6">
          <a
            href={`/listings?mode=auction${effectiveCat ? `&cat=${effectiveCat}` : ""}${selectedSizes.length ? `&sizes=${selectedSizes.join(",")}` : ""}`}
            className={`text-xs font-black uppercase px-4 py-2 rounded-full border-2 border-ink ${
              mode === "auction" ? "bg-ink text-white" : "bg-white text-ink"
            }`}
          >
            🔨 Live Auctions
          </a>
          <a
            href={`/listings?mode=retail${effectiveCat ? `&cat=${effectiveCat}` : ""}${selectedSizes.length ? `&sizes=${selectedSizes.join(",")}` : ""}`}
            className={`text-xs font-black uppercase px-4 py-2 rounded-full border-2 border-ink ${
              mode === "retail" ? "bg-bubblegum text-white" : "bg-white text-ink"
            }`}
          >
            🛍️ Buy Now
          </a>
        ...
        <div className="mb-6">
          <div className="flex flex-wrap gap-2">
            ...
            {categories.map((c) => (
              <Link
                key={c.id}
                href={`/listings?mode=${mode}&cat=${c.slug}${selectedSizes.length ? `&sizes=${selectedSizes.join(",")}` : ""}`}
                className={`text-xs font-black uppercase px-3 py-1 rounded-full border-2 border-ink ${
        ...
        {listings.length === 0 ? (
          <p className="font-bold text-gray-500 uppercase">
            {mode === "retail" ? "No Buy Now items right now. Check back soon." : "No live auctions right now. See the upcoming showcase below."}
          </p>
        ) : (
                ...
                {session?.user && (
                  <div className="absolute top-2 right-2 z-10">
                   ...
                   <div className="text-center">
                     <span className="font-black block text-lg">{money(l.buyNowPrice ?? l.currentBid ?? l.startingBid)}</span>
                     <span className="text-xs uppercase font-black text-gray-500">
                       {isRetail ? (l.listingMode === "BOTH" ? `${l.bidCount} bids · buy now` : "Buy now") : `${l.bidCount} bids · ${endsIn(mins)} left`}
                     </span>
                   </div>
                   {l.size && (
        ...
        {(mode === "auction" || upcomingItems.length > 0) && <UpcomingSection items={upcomingItems} />}
      </div>
    </div>
  );
}
