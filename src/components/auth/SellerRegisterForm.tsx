...
function SubmitButton({ isReapply }: { isReapply: boolean }) {
  ...
  return (
    <button disabled={pending} className="w-full bg-ink text-white font-black uppercase text-lg py-4 rounded-2xl border-2 border-ink shadow-brut-lg hover:bg-acid hover:text-ink transition-colors disabled:opacity-60">
      {pending ? (isReapply ? "Re-submitting application..." : "Creating seller account...") : (isReapply ? "Re-submit Application" : "Create seller account")}
    </button>
  );
}
...
export default function SellerRegisterForm() {
  ...
  return (
    <form action={formAction} className="space-y-5">
      <input type="hidden" name="reapply" value={isReapply ? "true" : "false"} />
      ...
      <SubmitButton isReapply={isReapply} />
